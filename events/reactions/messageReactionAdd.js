module.exports = (client, messageReaction, user) => {
    ...
    console.log(`Reaction ${emoji} added by ${member.nickname}`);
    ...
    if(["1️⃣", "2️⃣", "3️⃣", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣"].includes(emoji) && message.channel.id === channel1A.id) {
        switch (emoji) {
            case "1️⃣":
                ...
                channel_log.send(`Role ${Role_1A_TDA1.name} add to ${member.nickname}`);
                break;
            case "2️⃣":
                member.roles.add(Role_1A_TDA2);
                channel_log.send(`Role ${Role_1A_TDA2.name} add to ${member.nickname}`);
                break;
            case "3️⃣":
                member.roles.add(Role_1A_TDA3);
                channel_log.send(`Role ${Role_1A_TDA3.name} add to ${member.nickname}`);
                break;
            case "4️⃣":
                member.roles.add(Role_1A_TDA4);
                channel_log.send(`Role ${Role_1A_TDA4.name} add to ${member.nickname}`);
                break;
            case "5️⃣":
                member.roles.add(Role_1A_TDB1);
                channel_log.send(`Role ${Role_1A_TDB1.name} add to ${member.nickname}`);
                break;
            case "6️⃣":
                member.roles.add(Role_1A_TDB2);
                channel_log.send(`Role ${Role_1A_TDB2.name} add to ${member.nickname}`);
                break;
            case "7️⃣":
                member.roles.add(Role_1A_TDB3);
                channel_log.send(`Role ${Role_1A_TDB3.name} add to ${member.nickname}`);
                break;
            case "8️⃣":
                member.roles.add(Role_1A_TDSIB);
                channel_log.send(`Role ${Role_1A_TDSIB.name} add to ${member.nickname}`);
                break;
        };
    };
    ...
    if(["1️⃣", "2️⃣", "3️⃣", "4️⃣", "5️⃣", "6️⃣", "7️⃣"].includes(emoji) && message.channel.id === channel2A.id) {
        switch (emoji) {
            case "1️⃣":
                ...
                channel_log.send(`Role ${Role_2A_BLOIS_TD1.name} add to ${member.nickname}`);
                break;
            case "2️⃣":
                member.roles.add(Role_2A_BLOIS_TD2);
                channel_log.send(`Role ${Role_2A_BLOIS_TD2.name} add to ${member.nickname}`);
                break;
            case "3️⃣":
                member.roles.add(Role_2A_BLOIS_TD3);
                channel_log.send(`Role ${Role_2A_BLOIS_TD3.name} add to ${member.nickname}`);
                break;
            case "4️⃣":
                member.roles.add(Role_2A_BOURGES_TD1);
                channel_log.send(`Role ${Role_2A_BOURGES_TD1.name} add to ${member.nickname}`);
                break;
            case "5️⃣":
                member.roles.add(Role_2A_BOURGES_TD2);
                channel_log.send(`Role ${Role_2A_BOURGES_TD2.name} add to ${member.nickname}`);
                break;
            case "6️⃣":
                member.roles.add(Role_2A_BOURGES_TD3);
                channel_log.send(`Role ${Role_2A_BOURGES_TD3.name} add to ${member.nickname}`);
                break;
            case "7️⃣":
                member.roles.add(Role_2A_BOURGES_TD4);
                channel_log.send(`Role ${Role_2A_BOURGES_TD4.name} add to ${member.nickname}`);
                break;
        };
    };
    ...
    if(["1️⃣", "2️⃣", "3️⃣", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣", "🔟"].includes(emoji) && message.channel.id === channel3A.id) {
        switch (emoji) {
            case "1️⃣":
                ...
                channel_log.send(`Role ${Role_3A_BLOIS_TD1.name} add to ${member.nickname}`);
                break;
            case "2️⃣":
                member.roles.add(Role_3A_BLOIS_TD2);
                channel_log.send(`Role ${Role_3A_BLOIS_TD2.name} add to ${member.nickname}`);
                break;
            case "3️⃣":
                member.roles.add(Role_3A_BLOIS_TD3);
                channel_log.send(`Role ${Role_3A_BLOIS_TD3.name} add to ${member.nickname}`);
                break;
            case "4️⃣":
                member.roles.add(Role_3A_BLOIS_ALTERNANCE);
                channel_log.send(`Role ${Role_3A_BLOIS_ALTERNANCE.name} add to ${member.nickname}`);
                break;
            case "5️⃣":
                member.roles.add(Role_3A_BOURGES_MRI_TD1);
                channel_log.send(`Role ${Role_3A_BOURGES_MRI_TD1.name} add to ${member.nickname}`);
                break;
            case "6️⃣":
                member.roles.add(Role_3A_BOURGES_MRI_TD2);
                channel_log.send(`Role ${Role_3A_BOURGES_MRI_TD2.name} add to ${member.nickname}`);
                break;
            case "7️⃣":
                member.roles.add(Role_3A_BOURGES_MRI_TD3);
                channel_log.send(`Role ${Role_3A_BOURGES_MRI_TD3.name} add to ${member.nickname}`);
                break;
            case "8️⃣":
                member.roles.add(Role_3A_BOURGES_STI_TD1);
                channel_log.send(`Role ${Role_3A_BOURGES_STI_TD1.name} add to ${member.nickname}`);
                break;
            case "9️⃣":
                member.roles.add(Role_3A_BOURGES_STI_TD2);
                channel_log.send(`Role ${Role_3A_BOURGES_STI_TD2.name} add to ${member.nickname}`);
                break;
            case "🔟":
                member.roles.add(Role_3A_BOURGES_STI_TD3);
                channel_log.send(`Role ${Role_3A_BOURGES_STI_TD3.name} add to ${member.nickname}`);
                break;
        };
    };
}