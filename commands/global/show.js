...
module.exports.run = (client, message, args) => {
  if (args[0] === 'roles') {
    ...
    channels.forEach((channel) => {
      ...
      const rolesForChannel = roles.filter((item) =>
        item.name.match(rolesFilter)
      )
      let allRoles = ''
...
module.exports.help = {
  name: 'show',
  description:
    'Used to print cards to show content\n\nAvailable commands:\n- roles, to show all roles in corresponding channels\n```sh\n# Use show command\n$ Yui show <cmd>\n```',
  args: true,
  admin: true,
}
